...
from test_lanenet_final_rev05 import main_autonomous_loop
import pyautogui
...
postprocessor = lanenet_postprocess.LaneNetPostProcessor() #cfg제거
# from lanenet_inference.global_config import CFG
...
def main_drive_loop(weights_path):
    ...
    sess_config.gpu_options.allow_growth = True  # 고정값으로 설정
    #sess_config.gpu_options.allow_growth = CFG.GPU.TF_ALLOW_GROWTH --> cfg는 주석처리
    sess = tf.Session(config=sess_config)

    with tf.variable_scope(name_or_scope='moving_avg'):
        #variable_averages = tf.train.ExponentialMovingAverage(CFG.SOLVER.MOVING_AVE_DECAY) --> cfg 주석처리
        variable_averages = tf.train.ExponentialMovingAverage(0.9999)  # MOVING_AVE_DECAY 제거하고 고정값
