...
'''
def apply_control(offset, threshold=40, slow_down_zone=50,):
    offset = np.clip(offset, -100, 100)  # 조향 한계 제한

    if abs(offset) > slow_down_zone:
        pyautogui.keyUp('w')  # 중앙에서 많이 벗어나면 감속
    else:
        pyautogui.keyDown('w')  # 직진
    ...
    # 일정 시간마다 w 키를 떼서 감속 유도
    frame_count += 1
    if frame_count % 15 == 0:  # 매 15프레임마다 감속
        pyautogui.keyUp('w')
        time.sleep(0.05)
        pyautogui.keyDown('w')
...
def apply_control(offset, threshold=40, slow_down_zone=50):
    ...
    # 2. offset 값 제한 (과도한 조향 방지)
    offset = np.clip(offset, -100, 100)

    # 3. 차선 중심에서 많이 벗어나면 감속
    if abs(offset) > slow_down_zone:
        pyautogui.keyUp('w')
    else:
        pyautogui.keyDown('w')

    # 4. 일정 프레임마다 잠깐 멈춰서 속도 조절
    if frame_count % 15 == 0:
        pyautogui.keyUp('w')
        time.sleep(0.05)
        pyautogui.keyDown('w')
